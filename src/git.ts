...
export interface BranchInfo {
  ...
  commitsAhead?: number;
  /** Commits on main not in current branch */
...
function getBranchDiffFiles(
  repoRoot: string,
  mainBranch: string,
): Promise<Array<{ path: string; kind: FileChangeKind; renamePath?: string }>> {
  return execAsync(`git diff --name-status ${mainBranch}`, {
    cwd: repoRoot,
    ...
    .then(({ stdout }) => {
      ...
      return lines.map(
        (line): { path: string; kind: FileChangeKind; renamePath?: string } => {
...
function getWorkingTreeStatus(repoRoot: string): Promise<{
  statusMap: Map<string, WorkingTreeStatus>;
  statusEntries: StatusEntry[];
}> {
  return execAsync("git status --porcelain -u", {
    cwd: repoRoot,
...
export async function getBranchInfo(): Promise<BranchInfo | null> {
  ...
  try {
    ...
    const [currentBranch, diffFiles, statusResult, ahead, behind, stashList] =
      await Promise.all([
        getCurrentBranch(repoRoot),
        getBranchDiffFiles(repoRoot, mainBranch),
        getWorkingTreeStatus(repoRoot),
        execAsync(`git rev-list --count ${mainBranch}..HEAD`, { cwd: repoRoot })
          .then(({ stdout }) => Number.parseInt(stdout.trim(), 10))
          .catch(() => 0),
        execAsync(`git rev-list --count HEAD..${mainBranch}`, { cwd: repoRoot })
          .then(({ stdout }) => Number.parseInt(stdout.trim(), 10))
          .catch(() => 0),
        execAsync("git stash list", { cwd: repoRoot })
          .then(
    ...
    const changedFiles = Array.from(byPath.values()).sort((a, b) =>
